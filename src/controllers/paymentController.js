...
async function createPaymentOrder(req, res) {
  ...
  // For local address handling (if address_id is a UUID string from frontend)
  let addressData = null;
  if (address_id) {
    try {
      const addressResult = await db.query('SELECT * FROM addresses WHERE id = $1 AND user_id = $2', [address_id, userId]);
      if (addressResult.rows.length > 0) {
        addressData = addressResult.rows[0];
      }
    } catch (error) {
  ...
  try {
    ...
    try {
      const orderResult = await client.query(
        `INSERT INTO orders (id, user_id, address_id, zone_id, status, total, delivery_date, delivery_slot, razorpay_order_id, payment_status, customer_name, customer_phone, delivery_address, delivery_locality, delivery_pincode, created_at, updated_at)
         VALUES ($1, $2, $3, $4, 'placed', $5, $6, $7, $8, 'pending', $9, $10, $11, $12, now(), now())
         RETURNING *`,
        [orderId, userId, address_id, addressData?.zone_id || null, grandTotal, delivery_date, delivery_slot, razorpayOrder.id, addressData?.recipient_name || 'Customer', addressData?.recipient_phone || '', addressData?.line1 || '', addressData?.locality || '', addressData?.pincode || '']
      );
...
async function verifyPayment(req, res) {
  ...
  try {
    ...
    if (orderCheck.rows.length > 0) {
      ...
      const addressCheck = await db.query('SELECT id FROM addresses WHERE id = $1', [addressId]);
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      const demoOrder = await db.query(
