...
// Body: { status, sub_status } — Updated for detailed order lifecycle
async function updateOrderStatus(req, res) {
  ...
  if (sub_status && !validSubStatuses.includes(sub_status)) {
    return res.status(400).json({ error: `Sub-status must be one of: ${validSubStatuses.join(', ')}` });
  }

  const result = await db.query(
    `UPDATE orders SET status = $1, sub_status = $2, updated_at = now(), delivered_at = CASE WHEN $1 = 'delivered' THEN now() ELSE delivered_at END WHERE id = $3 RETURNING *`,
...
async function getStats(req, res) {
  ...
  const pending = await db.query(
    `SELECT COUNT(*)::int AS count FROM orders WHERE status IN ('pending', 'confirmed')`
  );
  ...
  const totalPartners = await db.query(
    `SELECT COUNT(*)::int AS count FROM users WHERE role = 'vendor'`
  );
  const weeklyRevenue = await db.query(
    `SELECT COALESCE(SUM(total), 0) AS revenue
     FROM orders WHERE created_at::date >= $1 AND status IN ('delivered', 'fulfilled')`,
    [weekAgo]
  );
  ...
  const topProducts = await db.query(
    `SELECT p.name, COUNT(oi.id)::int AS orders
     FROM order_items oi
  ...
  const statusCounts = {};
  orderStatusBreakdown.rows.forEach(row => {
  ...
  res.json({
    ...
    totalPartners: totalPartners.rows[0].count,
...
async function listPartners(req, res) {
  try {
    const result = await db.query(
      `SELECT id, name, email, phone, role, partner_type, business_name, bio, locality, fssai_number, fssai_verified, specialty_tags, is_public, created_at
       FROM users 
       WHERE role = 'vendor'
...
async function addPartner(req, res) {
  const { name, phone, email, partner_type, password } = req.body;
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO users (name, phone, email, password_hash, role, partner_type, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, now(), now())
       RETURNING id, name, email, phone, role, partner_type`,
      [name, phone, email || null, passwordHash, 'vendor', partner_type || 'milk_van']
    );
...
async function deletePartner(req, res) {
  ...
  try {
    const result = await db.query(
      `DELETE FROM users WHERE id = $1 AND role = 'vendor' RETURNING *`,
      [id]
    );
...
async function warnPartner(req, res) {
  ...
  try {
    ...
    const updatedPartner = (await db.query('SELECT * FROM users WHERE id = $1 AND role = $2', [id, 'vendor'])).rows[0];
...
// GET /api/admin/staging-queue
// Returns orders in the staging queue for Category C (Specialized Dairy & Organics)
// These orders require manual assignment by admin
async function getStagingQueue(req, res) {
  try {
    const stagingOrders = await db.query(
      `SELECT o.*, u.name AS customer_name, u.phone AS customer_phone,
              a.line1, a.city, a.pincode, a.locality,
    ...
    const orders = stagingOrders.rows;
    for (const order of orders) {
      const itemsResult = await db.query(
    ...
    console.error('Error fetching staging queue:', err);
    res.status(500).json({ error: 'Failed to fetch staging queue' });
  }
}

// POST /api/admin/staging-queue/:orderId/assign
// Manually assign a staging queue order to a partner
async function assignStagingOrder(req, res) {
  try {
    ...
    // Verify partner exists and is of appropriate type
    ...
    if (result.rows.length === 0) {
      return res.status(409).json({ error: 'Order was already assigned to another partner' });
    }
    ...
    console.error('Error assigning staging order:', err);
